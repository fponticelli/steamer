...
TestAll.prototype = {
	testConsumer: function() {
		var consumer = steamer.consumers.AssertConsumer.ofArray([1,2]);
		consumer.onPulse(steamer.Pulse.Emit(1));
		consumer.onPulse(steamer.Pulse.Emit(2));
		consumer.onPulse(steamer.Pulse.End);
...
steamer.consumers = {};
steamer.consumers.AssertConsumer = function(iterator,afterEnd) {
	if(null != afterEnd) this.afterEnd = afterEnd; else this.afterEnd = function() {
	};
	this.pulses = iterator;
};
steamer.consumers.AssertConsumer.__name__ = ["steamer","consumers","AssertConsumer"];
steamer.consumers.AssertConsumer.ofPulses = function(arr,afterEnd) {
	return new steamer.consumers.AssertConsumer((function($this) {
		var $r;
		var _this = arr.concat([steamer.Pulse.End]);
		$r = HxOverrides.iter(_this);
		return $r;
	}(this)),afterEnd);
};
steamer.consumers.AssertConsumer.ofArray = function(arr,afterEnd) {
	return steamer.consumers.AssertConsumer.ofPulses(arr.map(function(v) {
		return steamer.Pulse.Emit(v);
	}),afterEnd);
};
steamer.consumers.AssertConsumer.prototype = {
	pulses: null
	,afterEnd: null
	,onPulse: function(pulse) {
		if(!this.pulses.hasNext()) return utest.Assert.fail("Received " + Std.string(pulse) + " but the expectation queue is empty",{ fileName : "AssertConsumer.hx", lineNumber : 23, className : "steamer.consumers.AssertConsumer", methodName : "onPulse"});
		var expect = this.pulses.next();
		utest.Assert.same(expect,pulse,null,null,{ fileName : "AssertConsumer.hx", lineNumber : 25, className : "steamer.consumers.AssertConsumer", methodName : "onPulse"});
		if(Type.enumEq(pulse,steamer.Pulse.End)) this.afterEnd();
	}
	,assertEmpty: function() {
		utest.Assert.isFalse(this.pulses.hasNext(),"Consumer expectation iterator is not empty",{ fileName : "AssertConsumer.hx", lineNumber : 31, className : "steamer.consumers.AssertConsumer", methodName : "assertEmpty"});
	}
	,assertNotEmpty: function() {
		utest.Assert.isTrue(this.pulses.hasNext(),"Consumer expectation iterator is empty",{ fileName : "AssertConsumer.hx", lineNumber : 34, className : "steamer.consumers.AssertConsumer", methodName : "assertNotEmpty"});
	}
	,__class__: steamer.consumers.AssertConsumer
};
steamer.producers = {};
